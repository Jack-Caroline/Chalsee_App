...
class HandleLog:
    ...
    def create_log():
        ...
        log.setLevel(LOGGING.get("level"))
        # 创建一个输出到文件的渠道
        fn = TimedRotatingFileHandler(os.path.join(LOGS_DIR, LOGGING.get("log_name")), when='d', interval=1,
                                      backupCount=7, encoding="utf-8")  # 设置文件轮转条件，文件名、单位、轮转时间、文件保存数量、编码格式
        fn.setLevel(LOGGING.get("fn_level"))
        log.addHandler(fn)
